 ...
 *
 * @method static AccessListProperty find($id, $columns = ['*']) desc
 * @method static Collection all($columns = ['*']) desc
 * @method static Collection findMany($ids, $columns = ['*']) desc
 * @method static AccessListProperty|Collection findOrFail($id, $columns = ['*']) desc
 */
class AccessListPropertyModelBase extends Model
{
    /**
     * Generated
     */
    ...
    {
        $this->setTable('access_list_properties');
        $this->setFillable(
            [
    ...
    public static function get_model_rules($rules = null, $object_id = null)
    {
        if ($rules == null)
        {
            $rules = array_merge(AccessListProperty::$baseRules, AccessListProperty::$rules);
        }
