 ...
 * Class Tenant
 *
 * @method static Tenant find($id, $columns = ['*']) desc
 * @method static Collection all($columns = ['*']) desc
 * @method static Collection findMany($ids, $columns = ['*']) desc
 * @method static Tenant|Collection findOrFail($id, $columns = ['*']) desc
 */
class TenantModelBase extends Model
{
    /**
     * Generated
     */
    ...
    public static $baseRules = [
        ...
        'tenant_industry_id' => 'sometimes|nullable|integer',
     ...
     * @return \Illuminate\Database\Eloquent\Relations\belongsTo
     **/
    public function tenantIndustry()
    {
        return $this->belongsTo(
    ...
    public static function get_model_rules($rules = null, $object_id = null)
    {
        if ($rules == null)
        {
            $rules = array_merge(Tenant::$baseRules, Tenant::$rules);
        }
