 ...
 *
 * @method static PermissionRole find($id, $columns = ['*']) desc
 * @method static Collection all($columns = ['*']) desc
 * @method static Collection findMany($ids, $columns = ['*']) desc
 * @method static PermissionRole|Collection findOrFail($id, $columns = ['*']) desc
 */
class PermissionRoleModelBase extends Model
{
    /**
     * Generated
     */
    ...
    {
        $this->setTable('permission_roles');
        $this->setFillable(
            [
    ...
    public static function get_model_rules($rules = null, $object_id = null)
    {
        if ($rules == null)
        {
            $rules = array_merge(PermissionRole::$baseRules, PermissionRole::$rules);
        }
