 ...
 * Class EntityTag
 *
 * @method static EntityTag find($id, $columns = ['*']) desc
 * @method static Collection all($columns = ['*']) desc
 * @method static Collection findMany($ids, $columns = ['*']) desc
 * @method static EntityTag|Collection findOrFail($id, $columns = ['*']) desc
 */
class EntityTagModelBase extends Model
{
    /**
     * Generated
     */
    ...
    public static $baseRules = [
        ...
        'entity_model' => 'sometimes|max:255',
    ...
    public static function get_model_rules($rules = null, $object_id = null)
    {
        if ($rules == null)
        {
            $rules = array_merge(EntityTag::$baseRules, EntityTag::$rules);
        }
