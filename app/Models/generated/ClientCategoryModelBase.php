 ...
 *
 * @method static ClientCategory find($id, $columns = ['*']) desc
 * @method static Collection all($columns = ['*']) desc
 * @method static Collection findMany($ids, $columns = ['*']) desc
 * @method static ClientCategory|Collection findOrFail($id, $columns = ['*']) desc
 */
class ClientCategoryModelBase extends Model
{
    /**
     * Generated
     */
    ...
    {
        $this->setTable('client_categories');
        $this->setFillable(
            [
    ...
    public static function get_model_rules($rules = null, $object_id = null)
    {
        if ($rules == null)
        {
            $rules = array_merge(ClientCategory::$baseRules, ClientCategory::$rules);
        }
