...
use \Chrisbjr\ApiGuard\Http\Middleware\ApiGuard as ApiGuardBase;
use ApiGuardAuth;
use App;
 ...
 */
class ApiGuard extends ApiGuardBase
{
    /**
     ...
     * @throws \Exception
     *
     * @todo fix this - start by comparing this method w/ its parent and you will see
     */
    public function handle(Request $request, Closure $next, $serializedApiMethods = null)
    {
        ...
        $apiLog = App::make(config('apiguard.models.apiLog', 'App\Waypoint\Models\ApiLog'));
        ...
        if ( ! empty($this->apiMethods[$method]['limits']))
        {
                ...
                else
                {
                    if ( ! $this->apiKey->ignore_limits)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                Log::warning(
                                    "[ApiGuard] The API key ID#{$this->apiKey->id} has reached the limit of {$keyLimit} in the following route: " . Route::currentRouteAction()
                                );
            ...
            if ( ! empty($limits['method']))
            {
                ...
                {
                    Log::warning(
                        "[ApiGuard] You defined a method limit to the " . Route::currentRouteAction() . " route but you did not set a valid number for the limit variable."
                    );
                }
                else
                {
