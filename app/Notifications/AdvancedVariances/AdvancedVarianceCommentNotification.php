...
class AdvancedVarianceCommentNotification extends AdvancedVarianceNotificationBase
{
    /**
     * @return void
     */
    ...
    public function toMail($NotifiableUserObj)
    {
        try
        {
            return (new MailMessage)
                ->subject(
                    'Advanced Variance has a new COMMENT for property ' . $this->getAdvancedVarianceObj()->property->name
                )
                ->greeting('Advanced Variance has a new COMMENT')
                ->line('Hi ' . $NotifiableUserObj->firstname . ', ')
                ->line(
                    'An advanced variance has a new COMMENT for property ' . $this->getAdvancedVarianceObj()->property->name .
                    ...
                    ' for account code ' . $this->getAdvancedVarianceObj()->client_coa_line_item_code
                )
                ->action(
                    'View Advanced Variance',
                    $this->getBaseNotificationUrl() .
                    '#/property/variance/reports/' . $this->getAdvancedVarianceObj()->id . '?pureid=' .
