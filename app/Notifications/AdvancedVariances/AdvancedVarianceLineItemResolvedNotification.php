...
class AdvancedVarianceLineItemResolvedNotification extends AdvancedVarianceNotificationBase
{
    /**
     * @return void
     */
    ...
    public function toMail($NotifiableUserObj)
    {
        try
        {
            $subject                  = $this->getAdvancedVarianceLineItemObj()->advancedVariance->property->name . "'s " .
                                        $this->get_period_text() . ' Variance Report has a new resolution';
             ...
             * @todo fix me
             */
            $greeting = $this->get_account_name_from_advanced_variance_line_item().' has been resolved';
            if($this->getAdvancedVarianceLineItemObj()->resolverUser)
            {
                $greeting                 = $this->getAdvancedVarianceLineItemObj()->resolverUser->getDisplayName() .
                                            ' resolved ' . $this->get_account_name_from_advanced_variance_line_item();
            }
