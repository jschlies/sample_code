 ...
 */
class AccessListUserPublicController extends BaseApiController
{
    ...
    /**
     * @param integer $client_id
     * @param array|integer $access_list_id_arr
     * @param integer $user_id
     * @return \Illuminate\Http\JsonResponse|null
     */
    public function destroyByUser($client_id, $access_list_id_arr, $user_id)
    {
        if ( ! is_array($access_list_id_arr))
        {
            $access_list_id_arr = explode(',', $access_list_id_arr);
        }

        /** @var Collection $AccessListUserObjArr */
        $AccessListUserObjArr =
            $this->AccessListUserRepositoryObj
                ->findWhere(
                    [
                        'user_id' => $user_id,
                    ]
        ...
        $access_list_user_id_result_arr =
            $AccessListUserObjArr
                ->map(
                    function (AccessListUser $AccessListUserObj) use ($access_list_id_arr)
                    {
                        if (in_array($AccessListUserObj->access_list_id, $access_list_id_arr))
                        {
                            ...
                            return $AccessListUserObj->id;
                        }
                    }
                )
                ->filter(
                    function ($access_list_user_id) use ($access_list_id_arr)
                    {
                        return $access_list_user_id;
                    }
                )
                ->toArray();

        return $this->sendResponse($access_list_user_id_result_arr, 'AccessListUser deleted successfully');
    }
