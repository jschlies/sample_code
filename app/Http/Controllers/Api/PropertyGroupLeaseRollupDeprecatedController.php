 ...
 * @codeCoverageIgnore
 */
class PropertyGroupLeaseRollupDeprecatedController extends BaseApiController
{
    ...
    public function indexForPropertyGroup(Request $Request, $client_id, $property_group_id)
    {
        ...
        $property_lease_rollup_arr = $PropertyLeaseRollupObjArr->toArray();

        $key      = 'indexForPropertyGroup_metadata_' . Lease::get_model_as_of_date()->format('Y') . '_' . Lease::get_model_as_of_date()->format('m') . '_' . $property_group_id;
        $metadata = $PropertyGroupLeaseRollupObj->getPreCalcValue($key);
        if ($metadata === null)
        {
            $metadata                     = $this->get_property_arr_suites_metadata(
            ...
            $metadata                     = stdToArray($metadata);
            $metadata['lease_as_of_date'] = Lease::get_model_as_of_date()->format('Y-m-d H:i:s');
            $PropertyGroupLeaseRollupObj->updatePreCalcValue(
                $key,
                $metadata
            );
        }
