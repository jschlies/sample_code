 ...
 */
class VariancePropertyController extends LedgerController
{
    ...
    protected function package_data(\Illuminate\Support\Collection $results)
    {
        if ($results->count() > 0)
        {
            foreach ($results->pluck('code')->unique() as $code)
            {
                if (
    ...
    {
        return DatabaseConnectionRepository::getLedgerDatabaseConnection($this->ClientObj)
    ...
    /**
     * @param Collection|\Illuminate\Support\Collection $results
     * @return bool
     */
